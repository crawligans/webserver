...
public class HW3TestClient extends GenericTest {
  ...
  void prompt() {
    ...
    System.out.println(
        "In another terminal window, please run 'java cis5550.test.HW3TestServer, and then hit Enter in this window to continue.");
  ...
  void runTests(Set<String> tests) throws Exception {
    ...
    if (tests.contains("http")) {
      try {
        ...
        if (!r.body().equals("" + i)) {
          testFailed("The server returned a response, but it was not the '" + i
              + "' we expected. Here is what we received instead:\n\n" + dump(r.body));
        }
        s.close();
        testSucceeded();
      } catch (Exception e) {
    ...
    if (tests.contains("sessionid")) {
      try {
        ...
        if (r.headers.get("set-cookie") == null) {
          testFailed(
              "The server does not appear to have returned a Set-Cookie header at all. Check the implementation of your session() method.");
        }
        s.close();
        testSucceeded();
      } catch (Exception e) {
    ...
    if (tests.contains("permanent")) {
      try {
        ...
        if (r.statusCode != 200) {
          testFailed("The server returned a " + r.statusCode
              + " response for the first request, but we were expecting a 200 OK. Here is what was in the body:\n\n"
              + dump(r.body));
        }
        if (r.headers.get("set-cookie") == null) {
          testFailed(
              "The server does not appear to have returned a Set-Cookie header for the first request, without the session cookie. Check the implementation of your session() method.");
        }
        if (!r.body().equals(val)) {
          testFailed("The server returned something other than the expected value, '" + val
              + "', which was:\n\n" + dump(r.body)
        ...
        if (!setCookie.contains("SessionID=")) {
          testFailed(
              "The Set-Cookie: header was supposed to set a cookie called 'SessionID', but it looks like it didn't - the header we got was '"
                  + setCookie + "'. Check your session ID code.");
        }
        ...
        if (r.statusCode != 200) {
          testFailed("The server returned a " + r.statusCode
              + " response for the second request, but we were expecting a 200 OK. Here is what was in the body:\n\n"
              + dump(r.body));
        }
        if (r.headers.get("set-cookie") != null) {
          testFailed(
              "The server appears to have returned a Set-Cookie header for the second request, even though we did send a Cookie header with the session ID. Check the code that looks up existing sessions.");
        }
        if (!r.body().equals(val)) {
          testFailed("The server returned something other than the expected value, '" + val
              + "', which was:\n\n" + dump(r.body)
    ...
    if (tests.contains("expire")) {
      try {
        ...
        if (r.headers.get("set-cookie") == null) {
          testFailed(
              "The server does not appear to have returned a Set-Cookie header for the third request. Check whether sessions are being expired correctly.");
        }
        s.close();
