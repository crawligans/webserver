...
public class SNIInspector {
  ...
  public void parseConnection(Socket conn) throws Exception {
    ...
    if (buffer[0] != 22) {
      throw new Exception(
          "Expected record of type 22 for handshake, but got record type " + buffer[0]);
    }
    ...
    System.arraycopy(buffer, 0, b, 0, recordLength);
    ...
    int handshakeLength =
        ((buffer[6] & 0xFF) << 16) | (((buffer[7] & 0xFF) << 8) | (buffer[8] & 0xFF));
    if (handshakeLength > (recordLength - 4)) {
      throw new Exception(
          "Handshake length is " + handshakeLength + ", but record length is only " + recordLength);
    }
    ...
    while (extensionBytesRemaining > 0) {
      ...
      System.arraycopy(b, pos + 0, b2, 0, extLen);
