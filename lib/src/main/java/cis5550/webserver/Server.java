...
public class Server implements AutoCloseable {
    ...
    private static Route staticRoute = Route.returnStatus(ResponseImpl.Status.NOT_FOUND);
    ...
    public static void get(String pattern, Route route) throws Exception {
        insertRoute(cis5550.webserver.Request.Method.GET, pattern, route);
        if (!isRunning()) {
            run();
        }
    }

    protected static void insertRoute(cis5550.webserver.Request.Method method, String pattern,
        Route route) {
        synchronized (getInstance()) {
            hostMethodRouteTables.compute(host, (h, methodRouteTables) -> {
                if (methodRouteTables == null) {
                    methodRouteTables = new EnumMap<>(cis5550.webserver.Request.Method.class);
                }
    ...
    public static void post(String pattern, Route route) throws Exception {
        insertRoute(cis5550.webserver.Request.Method.POST, pattern, route);
        if (!isRunning()) {
            run();
        }
    }
    ...
    public static void put(String pattern, Route route) throws Exception {
        insertRoute(cis5550.webserver.Request.Method.PUT, pattern, route);
        if (!isRunning()) {
            run();
        }
    }
    ...
    private void handleRequests(Socket clientSocket) throws Exception {
        ...
        while (connectionOpen && !clientSocket.isInputShutdown()) {
            ...
            cis5550.webserver.Request request = null;
            Route rt;
            ResponseImpl response = new ResponseImpl(HTTPVersion.HTTP1_1, ResponseImpl.Status.OK, null, null) {
                @Override
            ...
            if (req.length < 3) {
                rt = Route.returnStatus(ResponseImpl.Status.BAD_REQUEST);
                ...
                rt = getRoute(RequestImpl.Method.valueOf(request.requestMethod().toUpperCase()),
            ...
            try {
                ...
                responseBodyOverride = Route.returnStatus(ResponseImpl.Status.INTERNAL_SERVER_ERROR)
                    .handle(request, response);
                connectionOpen = false;
            }
    ...
    private Route getRoute(cis5550.webserver.Request.Method method, String host, String path) {
        Map<cis5550.webserver.Request.Method, RouteTable> hostTable = hostMethodRouteTables.get(
            host);
        if (hostTable == null) {
